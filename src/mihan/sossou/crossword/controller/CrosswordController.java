...
public class CrosswordController {
    ...
    @FXML
    private ListView<Clue> horizontalCluesListView;
    @FXML
    private ListView<Clue> verticalCluesListView;

    private Database database;
    private Crossword crosswordModel;
    // Garde une trace du TextField actuellement focus
    ...
    private static final String CSS_CLASS_CORRECT_GUESS = "correct-guess-cell";
    private static final String CSS_CLASS_CURRENT_CLUE = "current-direction-clue";
    ...
    private void initialize() {
        ...
        horizontalCluesListView.getSelectionModel()
        ...
        verticalCluesListView.getSelectionModel()
                             .selectedItemProperty()
                             .addListener(
                                     (obs, oldVal, newVal) -> {
                                         if (!internalClueSelectionChange) {
    ...
    public void loadPuzzle(int puzzleNumber) {
        ...
        try {
            ...
            crosswordModel.printSolution();
        } catch (Exception e) {
            System.err.println("Erreur lors du chargement de la grille #" + puzzleNumber);
            e.printStackTrace();
            showError("Impossible de charger la grille: " + e.getMessage());
            gridPane.getChildren()
                    .clear();
            horizontalCluesListView.getItems()
                                   .clear();
            verticalCluesListView.getItems()
                                 .clear();
        }
    }
    ...
    private void populateGrid() {
        ...
        int numRows = crosswordModel.getHeight();
        int numCols = crosswordModel.getWidth();
        ...
        if (square.isBlack()) {
            ...
            textField.textProperty()
                     .addListener((obs, oldVal, newVal) -> {
                         ...
                         textField.getStyleClass()
                                  .remove(CSS_CLASS_CORRECT_GUESS);
                     ...
                     .addListener((obs, wasFocused, isFocused) -> {
                         if (isFocused && !internalFocusChange) {
                             if (currentTextField != textField) {
                                 System.out.println(">>> Focus Listener déclenché pour (" + row + "," + col + ")");
    ...
    private void handleCellClick(MouseEvent event, TextField textField, int row, int col) {
        if (crosswordModel == null || crosswordModel.isBlackSquare(row, col)) return;
        System.out.println(">>> Click détecté sur (" + row + "," + col + ")");

        if (textField == currentTextField) {
            System.out.println("... Direction basculée sur: " + ( crosswordModel.isCurrentDirectionHorizontal() ? "Horizontal" : "Vertical" ));
            // Re-surligne et met à jour l'indice courant
            highlightWord();
            // Met à jour l'indice rouge
            updateCurrentDirectionClueHighlight();
        } else {
            System.out.println("... Focusing sur une cellule différente (" + row + "," + col + ")");
            // Appelle highlightWord et selectClues
            focusCell(textField, row, col);
        }
        event.consume();
    }
    ...
    private void handleGridKeyPress(KeyEvent event) {
        ...
        switch (code) {
            case UP:
                moved = moveToNextEditableCell(false, -1);
                break;
            case DOWN:
                moved = moveToNextEditableCell(false, 1);
                break;
            case LEFT:
                moved = moveToNextEditableCell(true, -1);
                break;
            case RIGHT:
                moved = moveToNextEditableCell(true, 1);
                break;
            ...
            case BACK_SPACE:
                if (currentTextField != null) {
                    ...
                    // Déclenche handleTextInput qui met à jour le modèle
                    currentTextField.setText("");

                    // 2. Déplace le curseur en arrière dans la direction OPPOSEE
                    boolean moveSuccess = moveToNextEditableCell(!horizontal, -1);
                    // Si bloqué, essaye même direction
                    if (!moveSuccess) {
    ...
    /**
     * Vérifie TOUTES les cases de la grille et applique le style vert si
     * la proposition correspond à la solution.
     * (Adapté de la version Label pour utiliser TextField)
     */
    private void checkWholeGrid() {
        if (crosswordModel == null) return;
        System.out.println("Vérification de toute la grille (Touche Entrée)...");

        // Itère sur toutes les lignes et colonnes
        for (int row = 0; row < crosswordModel.getHeight(); row++) {
            for (int col = 0; col < crosswordModel.getWidth(); col++) {
                // Vérifie si ce n'est pas une case noire
                if (!crosswordModel.isBlackSquare(row, col)) {
                    // Récupère proposition et solution depuis le modèle
                    char proposition = crosswordModel.getProposition(row, col);
                    char solution = crosswordModel.getSolution(row, col);
                    // Trouve le TextField correspondant dans l'UI
                    TextField cellTextField = findTextField(row, col);

                    if (cellTextField != null) {
                        // Enlève toujours le style avant de vérifier
                        cellTextField.getStyleClass().remove(CSS_CLASS_CORRECT_GUESS);
                        // Ajoute le style si la proposition est correcte ET non vide
                        if (proposition != ' ' && proposition != '\0' && Character.toUpperCase(proposition) == Character.toUpperCase(solution)) {
                            cellTextField.getStyleClass().add(CSS_CLASS_CORRECT_GUESS);
                        }
                    }
                }
            }
    ...
    /**
     * Raccourci pour trouver le TextField à la position spécifiée.
     * Retourne null si non trouvé ou si ce n'est pas un TextField.
     */
    private TextField findTextField(int row, int col) {
    ...
    private void focusCell(TextField textField, int row, int col) {
        if (textField == null || crosswordModel == null || crosswordModel.isBlackSquare(row, col)) {
            System.err.println("Tentative de focus sur une cellule invalide: " + row + "," + col);
            return;
        }
        if (textField == currentTextField) {
            ...
            // Déjà la cellule courante
            return;
        }
        System.out.println("--- Cellule de mise au point (" + row + "," + col + ") ---");
        clearFocusAndHighlight();
    ...
    private void ensureCellVisible(TextField cell) {
        ...
        double cellMinY = cell.getLayoutY();
        double cellHeight = cell.getHeight();
        double viewportHeight = gridScrollPane.getViewportBounds().getHeight();
        double gridHeight = gridPane.getHeight();
        double currentVvalue = gridScrollPane.getVvalue();
        double targetVvalue = currentVvalue;
        double viewportMinY = ( gridHeight - viewportHeight ) * currentVvalue;
        double viewportMaxY = viewportMinY + viewportHeight;
        if (cellMinY < viewportMinY) {
            targetVvalue = cellMinY / ( gridHeight - viewportHeight );
        }
        else if (cellMinY + cellHeight > viewportMaxY) {
            targetVvalue = ( cellMinY + cellHeight - viewportHeight ) / ( gridHeight - viewportHeight );
        }
        double cellMinX = cell.getLayoutX();
        ...
        double currentHvalue = gridScrollPane.getHvalue();
        double targetHvalue = currentHvalue;
        double viewportMinX = ( gridWidth - viewportWidth ) * currentHvalue;
        double viewportMaxX = viewportMinX + viewportWidth;
        if (cellMinX < viewportMinX) {
            targetHvalue = cellMinX / ( gridWidth - viewportWidth );
        }
        else if (cellMinX + cellWidth > viewportMaxX) {
            targetHvalue = ( cellMinX + cellWidth - viewportWidth ) / ( gridWidth - viewportWidth );
        }
        if (Math.abs(targetVvalue - currentVvalue) > 1e-6) {
            gridScrollPane.setVvalue(Math.max(0, Math.min(1, targetVvalue)));
        }
        if (Math.abs(targetHvalue - currentHvalue) > 1e-6) {
            gridScrollPane.setHvalue(Math.max(0, Math.min(1, targetHvalue)));
        }
    }

    private void clearFocusAndHighlight() {
        if (currentTextField != null)
            currentTextField.getStyleClass().remove(CSS_CLASS_FOCUSED_CELL);
        for (Node node : gridPane.getChildren()) {
            if (node instanceof TextField) {
                node.getStyleClass()
                    .removeAll(CSS_CLASS_HIGHLIGHTED_CELL, CSS_CLASS_FOCUSED_CELL, CSS_CLASS_CORRECT_GUESS);
            }
        }
    ...
    private void highlightWord() {
        ...
        for (Node node : gridPane.getChildren()) {
            if (node instanceof TextField) node.getStyleClass()
                                               .remove(CSS_CLASS_HIGHLIGHTED_CELL);
        }

        int startR = r, startC = c;
        while (crosswordModel.correctCoords(startR, startC) && !crosswordModel.isBlackSquare(startR, startC)) {
            if (horizontal) {
                if (!crosswordModel.correctCoords(startR, startC - 1) || crosswordModel.isBlackSquare(startR,
                                                                                                      startC - 1))
                    break;
                startC--;
            } else {
                if (!crosswordModel.correctCoords(startR - 1, startC) || crosswordModel.isBlackSquare(startR - 1,
                                                                                                      startC)) break;
                startR--;
            }
        }
        ...
        while (crosswordModel.correctCoords(currentR, currentC) && !crosswordModel.isBlackSquare(currentR, currentC)) {
            ...
            if (cellNode instanceof TextField) {
                if (!cellNode.getStyleClass().contains(CSS_CLASS_HIGHLIGHTED_CELL))
                    cellNode.getStyleClass().add(CSS_CLASS_HIGHLIGHTED_CELL);
            }
            if (horizontal) currentC++;
            else currentR++;
        }

        // Réapplique le focus si nécessaire
        if (currentTextField != null) {
            if (!currentTextField.getStyleClass().contains(CSS_CLASS_FOCUSED_CELL))
                currentTextField.getStyleClass().add(CSS_CLASS_FOCUSED_CELL);
        }
    }

    private void populateClueLists() {
        horizontalCluesListView.getItems().clear();
        verticalCluesListView.getItems().clear();
    ...
    private Clue findClueContaining(ObservableList<Clue> clues, int row, int col, boolean horizontal) {
        ...
        for (Clue clue : clues) {
            if (clue.isHorizontal() == horizontal) {
                int startRow = clue.getRow();
                int startCol = clue.getColumn();
    ...
    private int getWordLength(int startRow, int startCol, boolean horizontal) {
        if (crosswordModel == null ||
                !crosswordModel.correctCoords(startRow, startCol) ||
                crosswordModel.isBlackSquare(startRow,startCol))
            return 0;
        int length = 0;
        int r = startRow;
        int c = startCol;
    ...
    private void handleClueSelection(Clue selectedClue, boolean horizontal) {
        ...
        System.out.println(">>> handleClueSelection déclenché pour: " + selectedClue + " (Horizontal: " + horizontal + ")");
        int startRow = selectedClue.getRow();
        int startCol = selectedClue.getColumn();
    ...
    private void handleLoadPuzzle() {
        if (database == null) {
            showError("BD non disponible.");
            return;
        }
        int maxGrid = database.gridSize();
        if (maxGrid <= 0) {
            showError("Aucun puzzle n'a été trouvé dans la BD.");
            return;
        }
        ...
        result.ifPresent(numStr -> {
            try {
                ...
                showError("Entrée invalide. Veuillez entrer un nombre.");
            } catch (Exception e) {showError("Erreur lors du chargement de la grille: " + e.getMessage());}
        });
    }

    @FXML
    private void handleLoadRandomPuzzle() { /* ... (inchangé) ... */
        if (database == null) {
            showError("La base de données n'est pas disponible.");
            return;
        }
        int maxPuzzles = database.gridSize();
        if (maxPuzzles <= 0) {
            showError("Aucun crossword n'est disponible dans la BD.");
            return;
        }
    ...
    @FXML
    private void handleAbout() { /* ... (inchangé) ... */
        ...
        aboutAlert.setContentText(
                "Développé par SOSSOU Horacio & MIHAN Zlanca-Nto.\nBasé sur les exigences du TP6 PRA (2024/25).\nUtilise JavaFX.");
